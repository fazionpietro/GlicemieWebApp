...
export function StatusBadge({ id, rilevazioni }: Props) {
  ...
  function getLevel() {
    const sum = rilevazioni?.filter((item) => item.idPaziente == id).reduce((a, c) => a + c.valore, 0)
    let avg = -1;
    if (rilevazioni && sum)
      avg = sum / rilevazioni?.length
    if (avg > 180)
      return 'alto'
    else if (avg < 70 && avg > 0)
      return 'basso'
    else if (avg <= 180 && avg >= 70)
      return 'normale'
    else
      return 'loading'
  }
  ...
  return (
    <>
      {(rilevazioni?.find((i) => i.idPaziente == id)) ? (
        <Badge size="sm" variant="filled" w="40%" bg={getColor(getLevel())}>
          {getLevel()}
        </Badge>
      ) : (
        <Badge size="sm" variant="filled" w="40%" bg={"gray"}>
          no rilevazioni
        </Badge>
      )}
    </>
...
export function RilevazioniModal({ id, rilevazioni, fetchRilevazioni }: Props) {
  ...
  const getRilevazioneStatus = (value: number) => {
    if (value > 180) return { status: 'Alto', color: theme.colors.red[6], icon: <IconArrowUpRight size={14} /> };
    if (value < 70) return { status: 'Basso', color: theme.colors.yellow[6], icon: <IconArrowDownRight size={14} /> };
    return { status: 'Normale', color: theme.colors.green[6], icon: <IconMinus size={14} /> };
  };

  // Calcola statistiche
  const latestValue = patientRilevazioni[0]?.valore || 0;
  const avgValue = patientRilevazioni.reduce((sum, r) => sum + r.valore, 0) / patientRilevazioni.length || 0;
  const maxValue = Math.max(...patientRilevazioni.map(r => r.valore), 0);
  const minValue = Math.min(...patientRilevazioni.map(r => r.valore), Infinity) || 0;
    ...
    <>
      ...
      >
        ...
        <Group justify="space-between" mb="lg">
          <Box>
            <Text size="sm" c="dimmed">Ultimo valore</Text>
            <Text size="xl" fw={700}>{latestValue} mg/dL</Text>
          </Box>
          <Box>
            <Text size="sm" c="dimmed">Media</Text>
            <Text size="xl" fw={700}>{avgValue.toFixed(1)} mg/dL</Text>
          </Box>
          <Box>
            <Text size="sm" c="dimmed">Massimo</Text>
            <Text size="xl" fw={700}>{maxValue} mg/dL</Text>
          </Box>
          <Box>
            <Text size="sm" c="dimmed">Minimo</Text>
            <Text size="xl" fw={700}>{minValue === Infinity ? 0 : minValue} mg/dL</Text>
          </Box>
        </Group>
        ...
        {/* Lista Rilevazioni */}
        <Title order={4} mb="sm">Storico Rilevazioni</Title>
        <ScrollArea h={300}>
