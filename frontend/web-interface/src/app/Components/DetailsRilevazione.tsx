...
/**
 * Component that displays a colored badge indicating the patient's glucose level status.
 */
export function StatusBadge({ id, rilevazioni }: Props) {
  ...
  function getLevel() {
    ...
    else if (avg < 80 && avg > 0) return "basso";
    else if (avg <= 180 && avg >= 80) return "normale";
    else return "loading";
  }
...
/**
 * Modal component to view detailed glucose readings history and statistics for a patient.
 */
  ...
  const patientRilevazioni =
    rilevazioni
      ...
      .map((i) => ({
        ...
        valore: i.valore,
        dopoPasto: i.dopoPasto,
      })) || [];

  // Funzione per determinare lo stato e il colore di una rilevazione
  // Logic matches backend RilevazioneUtenteDTO.calcoloLivello()
  const getRilevazioneStatus = (value: number, dopopasto: boolean) => {
    if (value > 180 || (value > 130 && !dopopasto) || value < 80) {
      return {
        status: "Alto",
              ...
              referenceLines={[
                {
                  y: 180,
                  color: "orange",
                  label: "Soglia massima dopo pasti",
                  strokeDasharray: "5 5",
                },
                {
                  y: 80,
                    ...
                    >
                      <Group justify="space-between">
                        <Box>
                          ...
                          <Text size="lg" fw={700}>
                            {item.valore} mg/dL{item.dopoPasto ? ' (dopo pasto)' : ''}
                          </Text>
                        </Box>
                        <Group gap="xs">
                          {icon}
