...
/**
 * Component that displays a table of patients.
 * Provides functionalities to view patient details, glucose readings, register new patients, and delete patients.
 */
export default function TablePazienti({
  ...
  const [openedRegister, { open: openRegister, close: closeRegister }] =
    useDisclosure(false);
  const [openedDel, { close: closeDel }] =
    useDisclosure(false);
  const { user } = useAuth();
