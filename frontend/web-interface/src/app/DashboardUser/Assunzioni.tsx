...
/**
 * Component for tracking medication adherence.
 * Displays daily therapy tasks and allows patients to mark medications as taken.
 */
function Assunzioni({ terapie, assunzioni, refreshComponente }: Props) {
  ...
  const handleCheck = (terapiaId: string) => {
    setSelezionaTerapia((pri) =>
      pri.includes(terapiaId)
        ? pri.filter((id) => id !== terapiaId)
        : [...pri, terapiaId],
    );
  };
  ...
  const handleInviaReport = async () => {
    ...
    try {
      const assunzioneDaInviare = selezionaTerapia.map((id) => ({
        idTerapia: id,
      }));

      const response = await fetch(
        `${import.meta.env.VITE_API_KEY}api/assunzioni/store`,
        {
          method: "POST",
  ...
  const noAssunzioni = useMemo(() => {
    ...
    const noAssunzioneDG = terapie.some((terapia) => {
      ...
      return (
        !assunzione?.latestTimestamp ||
        new Date(assunzione.latestTimestamp).getTime() < giorniSA
      );
    });
  ...
  return (
    <Box p="md" w="100%">
      ...
      <Stack gap="lg" mb="xl">
        {terapie.map((terapia) => {
          const assunzioneTerapia =
            assunzioni.find((assunzione) => assunzione.idTerapia === terapia.id)
              ?.giaAssunte ?? 0;
          ...
          return (
            <Card key={terapia.id} bdrs={"md"} withBorder shadow="sm" p="lg">
              <Grid>
                <Grid.Col span={7}>
                  ...
                  <Group gap="xs" mb="xs" style={{ flexWrap: "wrap" }}>
                    ...
                    <Text c="dimmed" size="sm">
                      {terapia.numAssunzioni} volt
                      {terapia.numAssunzioni > 1 ? "e" : "a"} al giorno
                    </Text>
                    {terapia.indicazioni && (
                      <Text
                        size="sm"
                ...
                <Grid.Col span={5}>
                  <Flex h="70%" justify="flex-end" align="center" pr={20}>
                    <Chip
                      color="green"
                      size="lg"
                      radius="md"
                      ...
                      onClick={() => handleCheck(terapia.id)}
                      disabled={booleanAssunzione}
                    >
                      Assumi
                    </Chip>
                  </Flex>
                  <Flex mt={10} h="30%" align={"self-end"} justify="flex-end">
                    {booleanAssunzione ? (
                      <Badge bg="green" size="sm" variant="filled">
                        Assunto
                      </Badge>
                    ) : (
                      <Text>
                        {assunzioneTerapia}/{terapia.numAssunzioni}
                      </Text>
                    )}
                  </Flex>
                </Grid.Col>
      ...
      {noAssunzioni === "non assumi farmaci da almeno 2 giorni" && (
        ...
        >
          <Text mb={20} fw={500}>
            Non assumi farmaci da almeno 2 giorni
          </Text>
        </Alert>
      )}
      ...
      {noAssunzioni === "ricordati di assumere i farmaci" && (
        ...
        >
          <Text mb={20} fw={500}>
            ricordati di assumere i farmaci
          </Text>
        </Alert>
      )}
