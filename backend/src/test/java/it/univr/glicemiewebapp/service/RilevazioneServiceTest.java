...
class RilevazioneServiceTest {
  ...
  void addRilevazione_ok() {
    ...
    Rilevazione result = rilevazioneService.addRilevazione(pazienteId, valore, false);

    assertNotNull(result);
    verify(pazienti).findById(pazienteId);
    verify(rilevazioni).save(any(Rilevazione.class));
    verify(log).info(contains("ha inserito una rilevazione"));
  }
  ...
  void addRilevazione_PazienteNotFound() {
    ...
    IllegalArgumentException exception = assertThrows(IllegalArgumentException.class,
        () -> rilevazioneService.addRilevazione(pazienteId, valore, false));
