...
/**
 * Service for managing communications between patients and doctors.
 * Handles sending messages, retrieving messages for doctors, and marking them
 * as read.
 */
@Service
...
public class ComunicazioneService {
    ...
    public List<ComunicazioneMedicoDTO> getByMedico(UUID medico) {
        try {
            ...
            throw new BusinessException("DATA_RETRIEVAL_ERROR", "Failed to retrieve communications");
        }
    }
    ...
    public String markAsRead(UUID id) {
        ...
        try {
            ...
            c.setLetto(true);
            return "Communication (" + id + ") marked as read";
        } catch (Exception e) {

            throw new BusinessException("UPDATE_ERROR", "failed to mark communication as read");
        }
    }
}
